...
export function GraphToolbar({ viewMode, overlayMode, edgeView, zoom, layoutCustomized, truncated, edgeCountsTruncated = false, omittedNodes = 0, omittedEdges = 0, availableEdgeKinds = [], edgeKindCounts = new Map(), activeEdgeKinds = new Set(), showUnprovenEdges = true, hideUnresolved = false, onViewMode, onOverlayMode, onEdgeView, onToggleEdgeKind = () => {}, onShowUnprovenEdges = () => {}, onHideUnresolved = () => {}, onResetLayout, onZoom, onRaiseBudget }: { viewMode: ViewMode; overlayMode: OverlayMode; edgeView: EdgeView; zoom: number; layoutCustomized: boolean; truncated: boolean; edgeCountsTruncated?: boolean; omittedNodes?: number; omittedEdges?: number; availableEdgeKinds?: string[]; edgeKindCounts?: ReadonlyMap<string, number>; activeEdgeKinds?: Set<string>; showUnprovenEdges?: boolean; hideUnresolved?: boolean; onViewMode: (mode: ViewMode) => void; onOverlayMode: (mode: OverlayMode) => void; onEdgeView: (mode: EdgeView) => void; onToggleEdgeKind?: (kind: string) => void; onShowUnprovenEdges?: (show: boolean) => void; onHideUnresolved?: (hide: boolean) => void; onResetLayout: () => void; onZoom: (zoom: number) => void; onRaiseBudget: () => void }) {
  return <>
    <div aria-label="Graph controls" className="absolute top-3 left-3 z-10 flex max-w-[calc(100%-1.5rem)] flex-wrap items-center gap-2">
      ...
      <label className="flex items-center gap-1 rounded border px-2 py-1 text-[10px]" style={{ borderColor: 'var(--atlas-border-strong)', background: 'var(--atlas-panel-header)' }}><input type="checkbox" checked={hideUnresolved} onChange={(event) => onHideUnresolved(event.target.checked)} />Hide unresolved nodes</label>
